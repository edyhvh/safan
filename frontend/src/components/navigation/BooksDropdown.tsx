...
import { AVAILABLE_BOOKS, BOOK_DISPLAY_NAMES, loadBookClient, type BookName } from '@/lib/books';
...
export default function BooksDropdown({ isOpen, onClose }: BooksDropdownProps) {
  ...
  useEffect(() => {
    if (hoveredBook && !(hoveredBook in loadedBooks) && !loadingBooks.has(hoveredBook)) {
