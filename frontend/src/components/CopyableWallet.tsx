...
export default function CopyableWallet({
  ...
  return (
    <div className="flex items-center justify-center gap-3 text-lg group">
      {icon}
      <span className="font-medium !text-black dark:!text-[#d5c4a1]">{name}</span>
      <button
        onClick={handleCopy}
        className="flex items-center gap-2 !text-black dark:!text-[#bdae93] hover:!text-black dark:hover:!text-[#d5c4a1] transition-colors cursor-pointer"
        title={`Copy ${name} address`}
      >
        <span className="font-mono text-sm !text-black dark:!text-[#bdae93]">{address}</span>
        {copied ? (
          <CheckmarkIcon className="w-4 h-4 text-green-600" />
        ) : (
          <CopyIcon className="w-4 h-4 !text-black dark:!text-[#bdae93] opacity-0 group-hover:opacity-100 transition-opacity" />
        )}
      </button>
    </div>
  )
