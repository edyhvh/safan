...
export async function loadBookServer(bookName: BookName): Promise<Book | null> {
  ...
  try {
    ...
    const publicDataPath = path.join(process.cwd(), 'public', 'data', `${bookName}.json`);
    const outputPath = path.join(process.cwd(), '..', 'output', `${bookName}.json`);
    ...
    if (!fs.existsSync(publicDataPath)) {
      if (fs.existsSync(outputPath)) {
        ...
        logger.error(`Book file not found: ${bookName}.json`, undefined, { bookName });
        return null;
      }
    }
